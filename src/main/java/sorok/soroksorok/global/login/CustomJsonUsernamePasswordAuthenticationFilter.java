...
public class CustomJsonUsernamePasswordAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
  ...
  private static final String DEFAULT_LOGIN_REQUEST_URL = "/api/auth/sign-in"; // "/api/auth/sign-in"으로 오는 요청을 처리
  ...
  private static final AntPathRequestMatcher DEFAULT_LOGIN_PATH_REQUEST_MATCHER =
      new AntPathRequestMatcher(DEFAULT_LOGIN_REQUEST_URL, HTTP_METHOD); // "/api/auth/sign-in" + POST로 온 요청에 매칭된다.

  private final ObjectMapper objectMapper;

  public CustomJsonUsernamePasswordAuthenticationFilter(ObjectMapper objectMapper) {
    super(DEFAULT_LOGIN_PATH_REQUEST_MATCHER); // 위에서 설정한 "/api/auth/sign-in" + POST로 온 요청을 처리하기 위해 설정
